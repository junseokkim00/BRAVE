...
with open('./2wikimh_revised/train.json', 'r') as f:
    train_dataset = json.load(f)

new_data_dict=defaultdict(list)
for idx, inst in enumerate(train_dataset):
    new_data_dict['idx'].append(idx)
    new_data_dict['question'].append(inst['question'])
    new_data_dict['context'].append(inst['context'])
    new_data_dict['supporting_facts'].append(inst['supporting_facts'])
    new_data_dict['answer'].append(inst['answer'])
train_dataset = Dataset.from_dict(new_data_dict)
...
for i in range(20):
    ...
    if max_em < cnt/ total_cnt * 100:
        ...
        model.save_pretrained('./bart-base-2wiki')
